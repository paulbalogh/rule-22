 ...
 * It runs a couple of generations of the game of life on the blocks.
 * Each generation the blocks are updated by the following rules:
 * - each block is evaluated based on the state of the block on the Left, the state of the current block and the block on the Right.
 * - if these three blocks contain one and only one 1, the current block becomes 1 in the next generation.
 * - otherwise, the current block becomes 0 in the next generation.
 * Boundary conditions assume that the missing (first or last) block is 0.
 * It takes the following props:
 * - the total number of items (how many blocks to display), default is 30 (show 10 per row)
 * - how many of the initial blocks are 1 (assigned randomly), default 1 block
 * - how many generations to run, default 10
...
import { useRule22 } from "./useRule22";
...
export function Rule22({
  ...
  }, [config.delay, config.generations, config.initialOnes, config.totalItems]);
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-0.5 text-xs text-slate-600 dark:text-slate-300">
              Rule: next cell is 1 iff exactly one of (L, C, R) is 1.
            </div>
          </div>
